...
const RecipeDetailScreen = ({ navigation, route }) => {
 ...
 const handleGenerateShoppingList = () => {
  ...
  const handleAIExplain = async () => {
    ...
    setIaResponse('');
    try {
        const recipeTitle = recipe.name || 'ce plat';
        const recipeTime = recipe.time || 'un certain temps';
        const ingredientList = scaledIngredients.map(ing => ing.name).join(', ') || 'des ingrédients inconnus';

      const prompt = `Explique simplement le plat "${recipe.title}". Il contient ${scaledIngredients.map(ing => ing.name).join(', ')} et prend ${recipe.time} à cuisiner.`;
      ...
      const response = await axios.post(
        ...
        {
          headers: {
            Authorization: 'Bearer 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', // Remplace ici ta vraie clé
